...
sequenceDiagram
  ...
  R1->>Cache: GET home
  Cache-->>R1: miss
  R1->>DB: query top stories
  R2->>Cache: GET home
  Cache-->>R2: miss
  R2->>DB: query top stories
  R3->>Cache: GET home
  Cache-->>R3: miss
  R3->>DB: query top stories
  Note over DB: concurrent identical queries overload the database
```
*Every request that misses in the same window starts its own rebuild instead of waiting for one.*
...
```mermaid
flowchart TD
  A["Request misses cache"] --> B{"Load already in flight for this key"}
  B -- No --> C["Create future and start loader"]
  C --> D["Write result to cache"]
  D --> E["Resolve future"]
  E --> F["All waiters receive the value"]
  B -- Yes --> G["Await the existing future"]
  G --> F
```
*Only the first miss for a key talks to the source; every other concurrent miss rides its future.*
