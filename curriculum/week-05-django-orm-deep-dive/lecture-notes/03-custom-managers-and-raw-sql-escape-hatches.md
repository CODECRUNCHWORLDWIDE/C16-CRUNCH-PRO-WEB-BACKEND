...
classDiagram
  ...
  ArticleQuerySet --> Manager : wrapped by as_manager
  Manager --> Article : attached as objects
```
*as_manager wraps a custom QuerySet's methods into the Manager that Article.objects exposes.*
...
```mermaid
flowchart TD
  A["ORM cannot express what you need"] --> B{"Just one raw expression inside a queryset?"}
  B -->|Yes| C["RawSQL"]
  B -->|No| D{"Need model instances back?"}
  D -->|Yes| E["objects.raw"]
  D -->|No| F{"Still inside a Django request?"}
  F -->|Yes| G["connection.cursor"]
  F -->|No| H["psycopg directly"]
```
*Choosing among the four raw-SQL escape hatches, in increasing order of you are on your own.*
