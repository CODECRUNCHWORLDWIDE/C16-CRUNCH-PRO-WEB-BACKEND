...
```mermaid
sequenceDiagram
  participant Outer as Outer query - authors
  participant Inner as Correlated subquery
  Outer->>Inner: OuterRef pk for this author row
  Inner->>Inner: Filter articles by author, order by published_at desc, limit 1
  Inner-->>Outer: Return single title value
  Outer->>Outer: Attach value as latest_article_title column
```
*How Subquery plus OuterRef resolves one correlated value per outer row.*
...
```mermaid
flowchart LR
  A["5 article rows"] --> B["GROUP BY category"]
  B --> C["2 rows out - reduced to one per category"]
  A --> D["Window partition by category"]
  D --> E["5 rows out - each keeps its row, plus a rank column"]
```
*GROUP BY collapses rows into groups; a window function keeps every row and adds a computed column per row.*
