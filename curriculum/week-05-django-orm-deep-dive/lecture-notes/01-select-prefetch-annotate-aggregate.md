...
```mermaid
flowchart TD
  A["Author queryset evaluated"] --> B["Query 1: select authors"]
  B --> C["Query 2: select articles where author_id in ids"]
  C --> D["Python: group articles by author_id"]
  D --> E["Attach matching list to each author in memory"]
```
*The two-query shape of prefetch_related: fetch parents, fetch children by IN, then join them in Python.*
