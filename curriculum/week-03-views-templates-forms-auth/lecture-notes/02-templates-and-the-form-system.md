...
```mermaid
flowchart TD
  Base["base.html defines block title and block content"] --> List["article_list.html extends base.html"]
  Base --> Detail["article_detail.html extends base.html"]
  List --> ListOverride["overrides block content with the article list markup"]
  Detail --> DetailOverride["overrides block content with the article detail markup"]
```
*Every page extends the one base template and overrides its named blocks.*
...
```mermaid
flowchart TD
  A["form.is_valid called"] --> B["Field-level cleaning to_python and validators"]
  B --> C["clean underscore fieldname methods run"]
  C --> D["form clean method sees all cleaned_data at once"]
  D --> E{"Any ValidationError raised"}
  E -->|Yes| F["Form is invalid errors attached to field or all"]
  E -->|No| G["Form is valid cleaned_data ready to use"]
```
*The order Django runs validation in every time is_valid is called.*
