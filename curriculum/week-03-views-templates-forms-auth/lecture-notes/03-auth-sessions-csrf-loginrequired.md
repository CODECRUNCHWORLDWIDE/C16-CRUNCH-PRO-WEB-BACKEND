...
sequenceDiagram
  ...
  User->>Bank: Log in and receive session cookie
  User->>Evil: Visit evil site in same browser
  Evil->>Bank: Auto submit hidden form browser attaches cookie
  Bank->>Bank: CsrfViewMiddleware checks hidden token against cookie
  Bank-->>Evil: Request rejected no matching csrf token
```
*The cookie rides along automatically, but the attacker cannot forge the matching CSRF token.*
...
sequenceDiagram
  ...
  Browser->>Middleware: GET dashboard new with session and csrf cookies
  Middleware->>Middleware: attach request.session and request.user
  Middleware->>View: dispatch call
  View->>View: LoginRequiredMixin check passes
  View->>Browser: render empty form with csrf token
  Browser->>Middleware: POST dashboard new with form data and token
  Middleware->>Middleware: CsrfViewMiddleware compares token to cookie
  Middleware->>View: dispatch call
  View->>View: bind form and run validation
  View->>DB: save the new article
  View->>Browser: redirect to article detail
```
*The full request cycle for a logged-in form POST, from the initial GET to the redirect.*
