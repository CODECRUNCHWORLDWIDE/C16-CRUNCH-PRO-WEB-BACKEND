...
sequenceDiagram
    participant Browser
    participant Server
    Browser->>Server: Open TCP connection
    Browser->>Server: Send GET request as plain text
    Server-->>Browser: Send status line and headers
    Server-->>Browser: Send response body
    Note over Browser,Server: Everything is just text over the wire
```

*The whole HTTP exchange, from TCP handshake to body, is just text sent back and forth.*
