...
sequenceDiagram
  ...
  App->>Conn: acquire for tenant acme
  App->>PG: SET app.current_tenant acme without LOCAL
  App->>PG: query as acme
  PG-->>App: acme rows
  App->>Conn: release to pool
  App->>Conn: acquire for tenant globex
  App->>PG: query as globex, no SET this time
  Note over PG: app.current_tenant still acme
  PG-->>App: acme rows returned to globex
```
*Without SET LOCAL, tenant context survives past the transaction and leaks to the next request on the same pooled connection.*
...
```mermaid
flowchart TD
  Root["RLS looks enabled but is not enforced"] --> G1["Superuser or BYPASSRLS role"]
  Root --> G2["Table owner without FORCE"]
  Root --> G3["SECURITY DEFINER function owned by a bypass role"]
  Root --> G4["Join query with surprising planner behavior"]
```
*Four distinct ways a policy that looks correct still fails to isolate tenants.*
