...
```mermaid
flowchart TD
  A["Sunday morning: final exam"] --> B["Sunday afternoon: walkthrough recording"]
  B --> C["Monday morning: practice defence answers"]
  C --> D["First week off curriculum: rest"]
  D --> E["First week back: choose next step"]
  E --> F["C17 advanced track"]
  E --> G["Job application"]
  E --> H["Own side project"]
```
*The wrap-up week runs in order: exam, recording, practice, rest, then a fork into three next steps.*
