...
```mermaid
flowchart TD
  A["fly deploy builds new image"] --> B["One-shot machine runs entrypoint migrate"]
  B --> C{"Migration succeeds"}
  C -->|No| D["Deploy aborts and old image keeps serving"]
  C -->|Yes| E["Start new web api and worker machines"]
  E --> F["Drain old machines"]
```
*The release command gates every deploy on a successful migration before any new traffic is served.*
...
```mermaid
flowchart LR
  A["Push to main"] --> B["Tests workflow runs"]
  B --> C{"Tests pass"}
  C -->|Yes| D["Deploy workflow triggers"]
  C -->|No| E["Deploy skipped"]
  D --> F["flyctl deploy remote only"]
```
*The deploy workflow only fires after the tests workflow reports success.*
