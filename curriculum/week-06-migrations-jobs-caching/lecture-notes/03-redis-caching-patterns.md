...
```mermaid
flowchart TD
  A["Look up key in cache"] --> B{"Cache hit"}
  B -->|Yes| C["Return cached value"]
  B -->|No| D["Run the expensive query"]
  D --> E["Store result in cache"]
  E --> C
```
*The cache-aside contract: check first, compute only on miss, then store.*
...
sequenceDiagram
  ...
  R1->>C: get key, miss
  R1->>C: add lock, succeeds
  R2->>C: get key, miss
  R2->>C: add lock, fails
  R1->>C: compute and set value
  R1->>C: delete lock
  R2->>C: retry get key, hit
```
*Only the lock winner recomputes the value; the loser waits and re-reads.*
