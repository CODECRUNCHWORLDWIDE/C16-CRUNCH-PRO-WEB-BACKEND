...
```mermaid
flowchart TD
  A["list_articles"] --> B["get_session"]
  A --> C["get_current_user"]
  C --> D["get_token"]
  C --> B
```
*get_session is resolved once and shared wherever it appears in the dependency graph.*
...
```mermaid
flowchart TD
  A["uvicorn builds scope receive send"] --> B["Middleware stack inbound"]
  B --> C["Router matches route"]
  C --> D["Parse body and solve dependencies"]
  D --> E["Call endpoint function"]
  E --> F["Validate and serialize response"]
  F --> G["Middleware stack outbound"]
  G --> H["Teardown dependencies and send response"]
```
*Every FastAPI request flows through these stages in order, from uvicorn in to uvicorn out.*
