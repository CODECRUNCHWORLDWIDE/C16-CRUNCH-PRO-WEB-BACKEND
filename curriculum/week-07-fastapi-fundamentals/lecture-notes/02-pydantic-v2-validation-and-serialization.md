...
classDiagram
  ...
  ArticleBase <|-- ArticleCreate
  ArticleBase <|-- ArticleRead
```
*ArticleCreate and ArticleRead both extend ArticleBase; ArticleUpdate stands alone with every field optional.*
...
```mermaid
flowchart TD
  A["Incoming JSON body"] --> B{"kind field"}
  B -->|email| C["Validate as EmailNotification"]
  B -->|sms| D["Validate as SMSNotification"]
  B -->|push| E["Validate as PushNotification"]
  B -->|missing or unknown| F["422 error"]
```
*The discriminator field routes validation straight to the matching variant instead of trying each in turn.*
