...
erDiagram
    ...
    ARTICLE }o--o{ CATEGORY : tags
    USER ||--|| PROFILE : has
```
*The three relationship shapes Django supports, mapped to the running examples.*
...
```mermaid
flowchart TD
    A["Parent row deleted"] --> B{"Does the child mean anything without the parent?"}
    B -- No --> C["CASCADE - delete child too"]
    B -- Yes --> D{"Should the delete be blocked?"}
    D -- Yes --> E["PROTECT - raise an error"]
    D -- No --> F["SET_NULL or SET_DEFAULT"]
```
*Choosing an on_delete option by asking what the child row means without its parent.*
